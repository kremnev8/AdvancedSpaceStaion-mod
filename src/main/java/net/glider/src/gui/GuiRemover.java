...
public class GuiRemover extends GuiContainer {
	...
	protected void actionPerformed(GuiButton button)
	{
		...
		if (button.id == 1)
		{
			...
			if (toDobjects.size() == 0)
			{
				...
				Minecraft.getMinecraft().thePlayer.addChatMessage(ChatUtils.modifyColor(new ChatComponentText(StatCollector.translateToLocal("remover.no_selected.name")), EnumChatFormatting.RED));
			}else
			{//sending packet
  ...
  /*  @Override
    public void drawScreen(int x, int y, float m)
    {
    	super.drawScreen(x, y, m);
    ...
    protected void drawGuiContainerForegroundLayer(int MouseX, int MouseY)
    {
    	...
    	/* if (buttonList.size() > 1)
         {
         int move = ((GuiVerticalSlider)buttonList.get(0)).getValueInt();
         ((GuiButton)buttonList.get(1)).yPosition = ((GuiButton)buttonList.get(1)).yPosition + (28*move);
         }*/
       
       fontRendererObj.drawString(StatCollector.translateToLocal("remover.name"), (int)(xSize /4.5D) - (fontRendererObj.getStringWidth(I18n.format("remover.name")) / 2)+15, 25, 4210752, false);
 
       if (hasChilds)
       {
    	  for (int i=0;i < Integer.parseInt(StatCollector.translateToLocal("remover.haschilds_tip.num_lines"));i++)
    	  {
    		  fontRendererObj.drawString(StatCollector.translateToLocal("remover.haschilds_tip"+i+".name")
    	        		, (int)(xSize /4.5D)-30
    	        		, 50+(i*9)
    	        		, 14737632
    	        		, true);  
       ...
       //   TileEntityInfo te = ((TileEntityRemoveInfo)world.getTileEntity(x, y, z)).infoBlock;
   //    fontRendererObj.drawString(te.xCoord+" "+te.yCoord+" "+te.zCoord, (int)(xSize /4.5D) - (fontRendererObj.getStringWidth(I18n.format("remover.name")) / 2)+15, 40, 4210752, false);
    }

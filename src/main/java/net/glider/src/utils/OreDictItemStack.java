...
public class OreDictItemStack {
	...
	public boolean isItemEqual(OreDictItemStack other)
	{
		...
		return other != null && (oreID.size() > 0 && ArrContain1SameInt(oreID, other.oreID) || (other.example != null && example.getItem() == other.example.getItem() && (!example.getHasSubtypes() || other.example.getItemDamage() == example.getItemDamage())));
	}
	
	public boolean isStackEqual(OreDictItemStack other, boolean ignoreStackSize)
	{

		return isItemEqual(other) && other.example != null && (example.stackSize <= other.example.stackSize || ignoreStackSize);
	}
	...
	public String toString()
	{
		...
		if (example != null)
		{
			ret = example.stackSize + "x" + example.getItem().getUnlocalizedName() + "@" + example.getItemDamage();
		}

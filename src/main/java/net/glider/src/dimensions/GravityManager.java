...
public class GravityManager extends SpinManager {
	...
	public GravityManager(WorldProviderSpaceStation provider)
	{
		super(provider);
	...
	public boolean checkSS(BlockVec3 baseBlock, boolean placingThruster)
	{
		...
		if (placingThruster && !this.checked.contains(baseBlock))
		{
			if (foundThrusters.size() > 0)
			{
				...
				if (ConfigManagerCore.enableDebug)
				{
					GCLog.info("Thruster placed on wrong part of space station: base at " + this.oneSSBlock.x + "," + this.oneSSBlock.y + "," + this.oneSSBlock.z
							+ " - baseBlock was " + baseBlock.x + "," + baseBlock.y + "," + baseBlock.z + " - found " + foundThrusters.size());
				}
				return false;
			}
	...
	public void updateSpin()
	{
		if (!this.clientSide)
		{
			...
			if (this.doSpinning)
			{
				...
				{
					if ((e instanceof EntityItem || e instanceof EntityLivingBase && !(e instanceof EntityPlayer) || e instanceof EntityTNTPrimed
							|| e instanceof EntityFallingBlock) && !e.onGround)
					{
						boolean freefall = true;
						if (e.boundingBox.maxX >= this.ssBoundsMinX && e.boundingBox.minX <= this.ssBoundsMaxX && e.boundingBox.maxY >= this.ssBoundsMinY
								&& e.boundingBox.minY <= this.ssBoundsMaxY && e.boundingBox.maxZ >= this.ssBoundsMinZ && e.boundingBox.minZ <= this.ssBoundsMaxZ)
						{
							...
							BLOCKCHECK: for (int x = MathHelper.floor_double(e.boundingBox.minX - 0.2D); x <= xmx; x++)
							{
								for (int y = ym; y <= yy; y++)
								{
									for (int z = zm; z <= zz; z++)

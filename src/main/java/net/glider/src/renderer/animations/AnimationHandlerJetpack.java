...
package net.glider.src.renderer.animations;
...
public class AnimationHandlerJetpack extends AnimationHandler {
	...
	public void fireAnimationEventClientSide(Channel anim, float prevFrame, float frame)
	{
		...
		/*		if(anim.name.equals("Enable") && frame >= 8)
				{
					ExtendedPlayer prop = (ExtendedPlayer) getEntity();
					if (prop.player.getCurrentArmor(2) != null && prop.player.getCurrentArmor(2).getItem() instanceof ItemSpaceJetpack)
					{
					ItemSpaceJetpack item = (ItemSpaceJetpack) prop.player.getCurrentArmor(2).getItem();
					item.setActive(true);
					}
				}
				if(anim.name.equals("Disable") && frame >= 8)
				{
					ExtendedPlayer prop = (ExtendedPlayer) getEntity();
					if (prop.player.getCurrentArmor(2) != null && prop.player.getCurrentArmor(2).getItem() instanceof ItemSpaceJetpack)
					{
	...
	{
		/*	if(channel.name.equals("Enable") && prevFrameAnim.get("Enable") >= 8.9)
			{
				this.stopAnimation("Enable");
				this.activateAnimation("Enabled idle", 0);
				return true;
			}
			if(channel.name.equals("Disable") && prevFrameAnim.get("Disable") >= 8.9)
			{
				this.stopAnimation("Disable");

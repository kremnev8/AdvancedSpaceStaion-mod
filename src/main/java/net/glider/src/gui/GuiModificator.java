...
public class GuiModificator extends GuiModular {
	...
	protected void drawGuiContainerBackgroundLayer(float time, int mX, int mY)
	{
		...
		{
			fontRendererObj.drawString(StatCollector.translateToLocal("modificator.children.name"),
					x + 7 + (int) (133 / 2) - (fontRendererObj.getStringWidth(StatCollector.translateToLocal("modificator.children.name")) / 2), NyPos, 4210752, false);
		}
		
		NyPos = text2y - (11 * move);
		if (NyPos > y - 25 || NyPos < y + 157)
		{
			fontRendererObj.drawString(StatCollector.translateToLocal("modificator.additional.name"),
					x + 7 + (int) (133 / 2) - (fontRendererObj.getStringWidth(StatCollector.translateToLocal("modificator.additional.name")) / 2), NyPos, 4210752, false);
		}

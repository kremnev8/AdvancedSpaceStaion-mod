...
public class MatrixHelper {
	...
	public static int[] findMatrixPoint(World world, ForgeDirection dir, int x, int y, int z)
	{
		if (dir != ForgeDirection.DOWN && dir != ForgeDirection.UP)
		{
			...
			if (world.getBlock(Px, Py, Pz) == BlockContainerMod.BlockInfo) return new int[] { Px, Py, Pz };
		}
		return null;
	}
	...
	public static Map<Integer, int[]> findTotalMatrix(World world, int[] Fpoint)
	{
		...
		if (Fpoint != null && Fpoint.length == 3)
		{
			...
			while (toScan.size() != 0)
			{
				...
				for (int i = 0; i < lastMatrix.size(); i++)
				{
					if (addIfNotAdded((int[]) lastMatrix.get(i))) addIfNotAddedToScan((int[]) lastMatrix.get(i));
				}
				MoveNextToPrev();
			}
	...
	public static void MoveNextToPrev()
	{
		...
		{
			if (toScan.containsKey(i + 1)) toScan.put(i, toScan.get(i + 1));
		}

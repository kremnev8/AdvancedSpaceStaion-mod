	...
	public static class Handler implements IMessageHandler<DeconstructPacket, IMessage>
	{
		...
		public IMessage onMessage(DeconstructPacket pkt, MessageContext ctx)
		{
			...
			if (pkt.objects != null && pkt.objects.size() > 0)
			{
				...
				if (!deconstruct) player.addChatMessage(ChatUtils.modifyColor(new ChatComponentText(StatCollector.translateToLocal("builder.deconstruct.failed")),EnumChatFormatting.RED));
				else player.addChatMessage(ChatUtils.modifyColor(new ChatComponentText(StatCollector.translateToLocal("builder.deconstruct.successfully")+"!"),EnumChatFormatting.GREEN));
